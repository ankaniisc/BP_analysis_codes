...
avgtime_consData = mean(consData,1)';          stderr_ConsData     = (std(consData))/sqrt(12);    
avgtime_alpaDepData = mean(alpahDepData,1)';   stderr_alpaDepData  = (std(alpahDepData))/sqrt(24);
avgtime_alphaIndData = mean(alphaIndData,1)';  stderr_alphaIndData = (std(alphaIndData))/sqrt(12);
average_alphaPowerAllTrials = mean(alphaPowerAllTrial,1); stderr_alphaPowerAllTrial = (std(alphaPowerAllTrial))/sqrt(48);

% acculmulating the datas in one single array;
% in the order : constand dependent independent

timepoint = 1:50;  % x axis total time duration of the experiment
% constrailno = [1,2,3,4];
...
figure('name','Change in alphapower from baseline (db)vs time');

subplot(2,3,1);
% plot(timepoint,avgtime_consData,'b-');
shadedErrorBar(timepoint,avgtime_consData,stderr_ConsData ,'b');
% set(gca,'XTickLabel',{'1','2','3','4'});
title('Contant feedback','FontSize',10,'FontWeight','bold','Color','k');
xlabel('Time(sec)','FontSize',8,'FontWeight','bold','Color','k'); ylabel('Change in alpha power from baseline (db)','FontSize',8,'FontWeight','bold','Color','k');
subplot(2,3,2);
% plot(timepoint,avgtime_alpaDepData,'b-');
shadedErrorBar(timepoint,avgtime_alpaDepData,stderr_alpaDepData ,'m');
...
shadedErrorBar(timepoint,avgtime_alphaIndData,stderr_alphaIndData ,'g');
...
% hc =  errorbar(timepoint,average_alphaPowerAllTrials,stderr_alphaPowerAllTrial,'.','MarkerSize',13,...
%     'MarkerEdgeColor','r','MarkerFaceColor','c','DisplayName','Constant trials'); hold on
% set(hc,'color','r'); hold on
shadedErrorBar(timepoint,average_alphaPowerAllTrials,stderr_alphaPowerAllTrial,'r');
% plot(timepoint,average_alphaPowerAllTrials,'k-');
% set(hc,'color','r')
% grid on
ax = gca;
ax.YMinorGrid = 'on';
...
xlabel('Time(sec)','FontSize',12,'FontWeight','bold','Color','k'); ylabel('Change in alpha power from baseline (db)','FontSize',12,'FontWeight','bold','Color','k');
title('Change in alpha power from baseline (db)  vs. time (average across all trials)','FontSize',14,'FontWeight','bold','Color','k');
